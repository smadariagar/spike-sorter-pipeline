...
import h5py
from scipy import signal  # Necesario para resampling científico

def get_channel_id_map(info_dset, max_channels=64):
    """
    Relaciona: ChannelID (Objetivo) -> RowIndex (Origen en el H5).
    """
...
def process_and_save_reordered(h5_dataset, reorder_map, output_path, fs_in, fs_out, chunk_size=400000):
    """
    Lee H5, reordena, aplica Resampling (Polyphase) y guarda en binario int16.
    """
    ...
    # resample_poly usa up/down para encontrar la relación racional
    up = int(fs_out)
    down = int(fs_in)
    
    print(f"  -> Guardando binario: {os.path.basename(output_path)}")
    if fs_in != fs_out:
        print(f"     [RESAMPLE] De {fs_in} Hz a {fs_out} Hz")

    with open(output_path, 'wb') as f_out:
        # Aumentamos el chunk_size para que el resampling sea más eficiente
        for start in range(0, total_samples, chunk_size):
            end = min(start + chunk_size, total_samples)
            current_chunk_width = end - start
            
            # 1. Leer bloque original
            raw_chunk = h5_dataset[:, start:end]
            
            # 2. Reordenar canales (64 filas)
            ...
            # 3. APLICAR RESAMPLING (Si las frecuencias son distintas)
            if fs_in != fs_out:
                # signal.resample_poly aplica filtro anti-aliasing automático
                ordered_chunk = signal.resample_poly(ordered_chunk, up, down, axis=1)
            
            # 4. Escalamiento y conversión a int16 (Formato Spike Sorting)
            ordered_chunk = np.rint(ordered_chunk * CONVERSION_FACTOR)
            np.clip(ordered_chunk, -32767, 32767, out=ordered_chunk)
            data_int16 = ordered_chunk.astype(np.int16)
            
            # 5. Transponer a (Samples x Channels) y escribir a disco
...
output_folder = os.path.join(input_folder, f"outputs_bin_{FS_TARGET/1000}kHz") 
